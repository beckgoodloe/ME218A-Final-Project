...
static const uint16_t InitValues[NUM_INIT_STEPS] = {
  ...
  (0x02 | USE_4_BIT_WRITE),  
    ...
    0x97 /* position cursor RIGHT SIDE OF SCREEN, SCROLL RIGHT TO LEFT*/};
...
static void LCD_RegisterSelect(uint8_t WhichReg){
  ...
  if (RegisterSelect)
  {
    SR_Write(CurrentValue | BIT1HI);
  }
  else {
    SR_Write(CurrentValue & BIT1LO);
  }
}
...
static void LCD_SetData4(uint8_t NewData){
  ...
  NextValue = ((CurrentValue & 0x0f) | NewData<<4);
